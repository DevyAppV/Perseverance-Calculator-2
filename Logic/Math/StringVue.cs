...
namespace Perseverance_Calculator_2.Logic.Math
{
    internal class StringVue
    {
        ...
        public static bool isMathExpression(string mathStr)
        {
            foreach (char c in mathStr)
            {
                switch (c)
                {
                    case '1': continue;
        ...
        public string replaceFormulaFunction(string mainStr, string functionToReplace = "formula(")
        {
            ...
            if (!string.IsNullOrWhiteSpace(mainStr))
            {
                ...
                //try
                //{
                while (mainStr.IndexOf(functionToReplace, indexOfFunc, StringComparison.CurrentCulture) > -1)
                {
                    indexOfFunc = mainStr.IndexOf(functionToReplace, StringComparison.CurrentCulture);
                    functionComma = indexOf(mainStr, ",", indexOfFunc + functionToReplace.Length, true);
                    functionEndPar = indexOf(mainStr, ")", indexOfFunc, true);

                    bool found = false;
                    if (indexOfFunc - 1 >= 0)
                    {
                        if ((
                            mv.isOperator(mainStr[indexOfFunc - 1]) ||
                            mv.isComma(mainStr[indexOfFunc - 1]) ||
                            mv.isParenthesis(mainStr[indexOfFunc - 1])
                            ))
                        {
                            found = true;
                            subStrFormulaName = mainStr.Substring(indexOfFunc + functionToReplace.Length, (functionComma - 1) - (indexOfFunc + functionToReplace.Length - 1));
                            subStrExpression = mainStr.Substring(functionComma + 1, functionEndPar - (functionComma + 1));
                            //int assd = mainStr.Length;
                            mainStr = mainStr.Remove(indexOfFunc, functionEndPar + 1 - indexOfFunc);
                    ...
                    else
                    {
                        ...
                        subStrFormulaName = mainStr.Substring(indexOfFunc + functionToReplace.Length, (functionComma - 1) - (indexOfFunc + functionToReplace.Length - 1));
                        subStrExpression = mainStr.Substring(functionComma + 1, functionEndPar - (functionComma + 1));
                        mainStr = mainStr.Remove(indexOfFunc, functionEndPar + 1);
                        mainStr = replaceData(mainStr, subStrFormulaName, subStrExpression).getNewData;
                    }
                    if (!found)
                        indexOfFunc++;
        ...
        public string replaceVarFunction(Formula formulaObj, string mainStr, string functionToGet = "var(")
        {
            ...
            if (!string.IsNullOrWhiteSpace(mainStr))
            {
                ...
                //try
                //{
                while (mainStr.IndexOf(functionToGet, indexOfFunc, StringComparison.CurrentCulture) > -1)
                {
                    indexOfFunc = mainStr.IndexOf(functionToGet, StringComparison.CurrentCulture);
                    functionComma = indexOf(mainStr, ",", indexOfFunc + functionToGet.Length, true);
                    functionEndPar = indexOf(mainStr, ")", indexOfFunc, true);
                    ...
                    if (indexOfFunc - 1 >= 0)
                    {
                            ...
                            ))
                        {
                            ...
                            found = true;
                            subStrFormulaName = mainStr.Substring(indexOfFunc + functionToGet.Length, (functionComma - 1) - (indexOfFunc + functionToGet.Length - 1));
                            subStrExpression = mainStr.Substring(functionComma + 1, functionEndPar - (functionComma + 1));
                            //int assd = mainStr.Length;
                            mainStr = mainStr.Remove(indexOfFunc, functionEndPar + 1 - indexOfFunc);
                            mainStr = replaceData(mainStr, subStrFormulaName, new MathVue<double>().solveFormula(formulaObj, subStrExpression, false)).getNewData;
                        }
